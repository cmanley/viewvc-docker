...
based on nginx and Alpine Linux 3.8, with configurable run-time options (theme, timezone, and group id).
...
You can simply pull this image from docker hub like this:
	...
	docker pull cmanley/viewvc:alpine-dev

### Option 2: Build the image yourself ###

	git clone <Link from "Clone or download" button>
	cd viewvc-docker
	docker build --rm -t cmanley/viewvc:alpine-dev .
...
Minimal:
	...
	docker run --name viewvc -v /var/lib/cvs:/opt/cvs:ro -p 127.0.0.1:8002:80/tcp --rm -d cmanley/viewvc:alpine-dev
...
Using both CVS and SVN repositories:
	...
	--rm -d cmanley/viewvc:alpine-dev
...
Start container and a shell session within it (this does not start nginx):
	...
	-v /var/lib/cvs:/opt/cvs:ro \
	-p 127.0.0.1:8002:80/tcp \
	--rm -it cmanley/viewvc:alpine-dev shell
